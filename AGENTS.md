...
There is currently no dedicated `public/` asset directory or test suite; add those directories when needed.
...
## Testing Guidelines
No automated testing framework or coverage requirement is currently configured. For every change, manually verify empty input, known junk and healthy foods, unknown foods, optional descriptions, preparation-based classification, and quick-example buttons.
If adding tests, use `*.test.js` filenames and place them beside the module or in a top-level `tests/` directory. Add the corresponding `npm test` script to `package.json`.
